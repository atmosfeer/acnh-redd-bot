...
class App < Sinatra::Base
  ...
  bot.command :bot_delete_message do |event|
    message_id = event.content.downcase.gsub("i!bot_delete_message", "").strip
    channel = bot.channel(ENV["CHANNEL_ID"])
    announcement = Announcement.find_by(discord_id: message_id)
    if event.user.id == ENV["SUPER_USER_ID"].to_i
  ...
  bot.command :bot_reset_announcements do |event|
